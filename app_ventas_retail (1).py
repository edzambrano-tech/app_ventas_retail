...
def main():
    ...
    """
    # Header principal
    st.markdown('<h1 class="main-header"> RetailMax - Predictor de Ventas</h1>',
    unsafe_allow_html=True)
...
"""Para ejecutar esta aplicación Streamlit en Colab, primero necesitas guardar todo el contenido de tu cuaderno en un archivo Python (por ejemplo, `app.py`). Luego, podrás ejecutarlo con el comando `streamlit run`."""

# Para guardar todo el cuaderno en un archivo .py, necesitaríamos consolidar todas las celdas en un solo script.
# Dado que tienes varias celdas que definen funciones, vamos a asumir que el código completo de la aplicación se ha reunido en un archivo llamado 'app.py'.
# Si este no fuera el caso, necesitarías copiar y pegar todo el código de las celdas anteriores en un solo string para 'streamlit_code'.

# Por ahora, para la demostración, si todas las celdas anteriores estuvieran en un solo script:
# Puedes simular esto guardando el contenido de las celdas relevantes si es necesario, o simplemente correr el comando Streamlit si el entorno lo permite directamente.

# Guardar el contenido de todas las celdas relevantes en un archivo temporal 'app.py'
...
!pip install streamlit pandas numpy plotly.express scikit-learn -q
